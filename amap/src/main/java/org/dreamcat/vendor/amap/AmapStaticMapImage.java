...
public class AmapStaticMapImage {
    ...
    public String getImageUrl() {
        ...
        if (ObjectUtil.isNotBlank(location)) s.append("&location=").append(location);
        if (ObjectUtil.isNotBlank(size)) s.append("&size=").append(size);
        if (scale != null) s.append("&scale=").append(scale);
        if (ObjectUtil.isNotEmpty(markers)) s.append("&markers=").append(strings(markers));
        if (ObjectUtil.isNotEmpty(labels)) s.append("&labels=").append(strings(labels));
        if (ObjectUtil.isNotEmpty(paths)) s.append("&paths=").append(strings(paths));
        if (traffic != null) s.append("&traffic=").append(traffic);
        return s.toString();
    }
